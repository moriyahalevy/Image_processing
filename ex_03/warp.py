...
def warp_image(image: np.ndarray,
    ...
    # 2. וקטוריזציה: יצירת רשת קואורדינטות לכל התמונה בבת אחת
    ys, xs = np.indices((H, W))
    
    # הוספת 0.5 לפי המודל והכנה להכפלה מטריציונית (הוספת שכבת 1-ים)
    coords = np.stack([xs.ravel() + 0.5, ys.ravel() + 0.5, np.ones(H*W)])
    
    # חישוב המקור לכל הפיקסלים בבת אחת
    ...
    # חישוב משקלי האינטרפולציה
    a = src_x - x0
    b = src_y - y0

    # ביצוע החישוב רק לפיקסלים שבתוך המסיכה
    for c in range(C):
        img_c = image[:, :, c]
        output[mask, c] = (
